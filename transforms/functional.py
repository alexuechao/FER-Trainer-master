...
def blur_perturb(img, param_conf):
    '''Blur image
    '''
    img = cv2.cvtColor(np.asarray(img),cv2.COLOR_RGB2BGR)
    if np.random.rand() < 0.8:
        img_new = gaussian_blur(img, param_conf)
        img_new = Image.fromarray(cv2.cvtColor(img_new,cv2.COLOR_BGR2RGB))
    else:
        length = np.random.randint(param_conf['length_min'], param_conf['length_max'])
        angle = np.random.randint(param_conf['angle_min'], param_conf['angle_max'])
        if angle in [0, 90, 180, 270, 360]:
            angle += 1
        k, a = motion_blur(img, length, angle)
        img_new = cv2.filter2D(img, -1, k, anchor=a)
        img_new = Image.fromarray(cv2.cvtColor(img_new,cv2.COLOR_BGR2RGB))

    return img_new

def gaussian_blur(img, param_conf):
    k = np.random.randint(param_conf['kernel_size_min'], param_conf['kernel_size_max'])
    if k % 2 == 0:
        if np.random.rand() > 0.5:
            k += 1
        else:
            k -= 1
    s = np.random.uniform(param_conf['sigma_min'], param_conf['sigma_max'])
    img_blur = cv2.GaussianBlur(img, (k, k), s)
    return img_blur
