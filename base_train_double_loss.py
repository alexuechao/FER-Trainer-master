...
class Trainer(object):
	def __init__(self, configs):
		...
		self.island_loss = self.configs['island_loss']
	...
	def train_model(self, net, criterion, criterion_island, optimizer, optimizer_island, scheduler,\
	                trainloader, valloader, testloader, logger, writer, path):
		use_cuda = torch.cuda.is_available()
		## if or not pre
		if self.pretrained:
			...
			for batch_idx, (inputs, targets) in enumerate(trainloader):
				...
				if self.mixup or self.cutmix:
					if epoch <=self.num_epochs - 20:
						...
						if self.island_loss:
							optimizer.zero_grad()
							optimizer_island.zero_grad()
							#feat, outputs = net(inputs)
							feat, outputs = net(x_batch.cuda())
							loss1 = mixup_criterion(criterion, outputs, y_batch_a.cuda(device=self.device_ids[0]), y_batch_b.cuda(device=self.device_ids[0]), lam)
							#loss2 = criterion_island(feat, targets)
							lam = torch.tensor(lam, dtype=torch.float32)
							targets_island = lam * y_batch_a + (1-lam)*y_batch_b
							loss2 = criterion_island(feat, targets_island)
							weight_cent = 1.0
							loss2 *= weight_cent
							loss = loss1 + loss2
							loss.backward()
							...
							optimizer.step()
							for param in criterion_island.parameters():
								param.grad.data *= (1./weight_cent)
							optimizer_island.step()
						else:
					...
					if self.island_loss:
						...
						for param in criterion_island.parameters():
							param.grad.data *= (1./weight_cent)
						optimizer_island.step()
						train_loss_islandloss += loss2.item()
						train_loss += loss.item()
					else:
						optimizer.zero_grad()
						outputs = net(inputs)
				...
				if batch_idx % 20 == 0:
					if self.island_loss:
						train_log = ('Epoch[%d/%d] Batch[%d] lr: %.6f, Training_Loss=%.4f, Train-IslandLoss=%.7f, Train_Accuracy=%.4f' %
							(epoch + 1, self.num_epochs, batch_idx, current_lr, train_loss / (batch_idx + 1), train_loss_islandloss/(batch_idx+1), accuracy))
					else:
	...
	def train(self):
		...
		criterion_island = IslandLoss(features_dim=2, num_class=7, alpha1=0.1, scale=1.0, batch_size=self.batch_size)
		## optimizer
		if self.optimizers == 'SGD':
			optimizer = optim.SGD(net.parameters(), lr=self.init_lr, momentum=0.9, weight_decay=self.weight_decay)
			optimizer_island = optim.SGD(criterion_island.parameters(),lr=self.init_lr,momentum=0.9, weight_decay=self.weight_decay)
		elif self.optimizers == 'Adam':
			optimizer = optim.Adam(net.parameters(), lr=self.init_lr, weight_decay=self.weight_decay)
			optimizer_island = optim.Adam(criterion_island.parameters(),lr=self.init_lr, weight_decay=self.weight_decay)
		...
		self.train_model(net, criterion, criterion_island, optimizer, optimizer_island, scheduler, trainloader, Valloader, Testloader, logger, writer, path)
