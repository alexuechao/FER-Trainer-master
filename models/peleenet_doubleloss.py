...
class PeleeNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
                 num_init_features=32, bottleneck_width=[1, 2, 4, 4], drop_rate=0.05, num_classes=7):
        ...
        # double loss:island loss or center loss
        self.relu = nn.ReLU(inplace=True)
        self.center_loss_fc = nn.Linear(704,2)
...
def Peleenet_doubleloss():
    return PeleeNet(growth_rate=32, block_config=[3, 4, 8, 6],num_init_features=32, 
                    bottleneck_width=[1, 2, 4, 4], drop_rate=0.05, num_classes=7)
